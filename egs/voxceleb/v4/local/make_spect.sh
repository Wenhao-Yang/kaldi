...
fbankdir=`pwd`/data/vox1_spect/feats
vaddir=`pwd`/data/vox1_spect/vads

# The trials file is downloaded by local/make_voxceleb1.pl.
train_dir=data/vox1_spect/train
test_dir=data/vox1_spect/test
stage=0
...
if [ $stage -le 1 ]; then
  ...
  for name in $train_dir $test_dir; do
    ...
    steps/make_spect.sh --write-utt2num-frames true --fbank_config conf/spect.conf --nj 4 --cmd "$train_cmd" \
        ${name} exp/make_vox1_spect $fbankdir
...
if [ $stage -le 2 ]; then
  ...
  local/nnet3/xvector/prepare_feats_for_egs.sh --nj 4 --cmd "$train_cmd" $train_dir data/vox1_spect/train_no_sli data/vox1_spect/feats_no_sil
  utils/fix_data_dir.sh data/vox1_spect/train_no_sli

  # This script applies CMVN and removes nonspeech frames.  Note that this is somewhat
  # wasteful, as it roughly doubles the amount of training data on disk.  After
  # creating training examples, this can be removed.
  local/nnet3/xvector/prepare_feats_for_egs.sh --nj 4 --cmd "$train_cmd" $test_dir data/vox1_spect/test_no_sli data/vox1_spect/feats_no_sil
  utils/fix_data_dir.sh data/vox1_spect/test_no_sli
fi
