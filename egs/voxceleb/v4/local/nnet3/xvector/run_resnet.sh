...
if [ $stage -le 7 ]; then
  ...
  conv-layer name=conv1 $a height-in=64 height-out=32 height-subsample-out=2 time-offsets=-2,-1,0,1,2 required-time-offsets=0 height-offsets=-2,-1,0,1,2 num-filters-out=$nf1

  res-block name=res1 num-filters=$nf1 height=32 time-period=1 $res_opts

  conv-layer name=conv2 height-in=32 height-out=16 height-subsample-out=2 time-offsets=-1,0,1 $common num-filters-out=$nf2

  res-block name=res2 num-filters=$nf2 height=16 time-period=2 $res_opts

  conv-layer name=conv3 height-in=16 height-out=8 height-subsample-out=2 time-offsets=-2,0,2 $common num-filters-out=$nf3

  res-block name=res3 num-filters=$nf3 num-bottleneck-filters=$nb3 height=8 time-period=4 $res_opts

  stats-layer name=stats config=mean+stddev(0:1:1:${max_chunk_size})

  relu-batchnorm-layer name=fc1 dim=512
...
EOF
  ...
  echo "output-node name=output input=fc1.affine" > $nnet_dir/extract.config
...
if [ $stage -le 8 ]; then
  steps/nnet3/train_raw_dnn.py --stage=$train_stage \
    --cmd="$train_cmd" \
    --trainer.optimization.proportional-shrink 1 \
